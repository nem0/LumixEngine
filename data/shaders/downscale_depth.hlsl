...
void main(uint3 thread_id : SV_DispatchThreadID, uint3 group_thread_id : SV_GroupThreadID) {
	...
	bindless_rw_textures[u_output0][thread_id.xy] = d;
	...
	if (all(group_thread_id % 2 == 0)) {
		...
		bindless_rw_textures[u_output1][thread_id.xy >> 1] = d;
		scratch_depths[group_thread_id.y][group_thread_id.x] = d;
	}
	...
	if (all(group_thread_id % 4 == 0)) {
		...
		bindless_rw_textures[u_output2][thread_id.xy >> 2] = downsampleFunc(d00, d10, d11, d01);
		scratch_depths[group_thread_id.y][group_thread_id.x] = d;
	}
	...
	if (all(group_thread_id % 8 == 0)) {
		...
		bindless_rw_textures[u_output3][thread_id.xy >> 3] = downsampleFunc(d00, d10, d11, d01);
	}
}
