...
static int LUA_destroyPipeline(lua_State* L) {
	...
	auto* pipeline = LuaWrapper::checkArg<Pipeline*>(L, 1);
	LUMIX_DELETE(renderer->getAllocator(), pipeline);
	return 0;
}
...
static void LUA_createComponent(lua_State* L, World* world, i32 entity, const char* type)
{
	...
	if (world->hasComponent(EntityRef{entity}, cmp_type)) {
		logWarning("Component ", type, " already exists in entity ", entity);
		return;
	}
...
void registerEngineAPI(lua_State* L, Engine* engine) {
	...
	LuaWrapper::createSystemFunction(L, "LumixAPI", "destroyPipeline", &LUA_destroyPipeline);
