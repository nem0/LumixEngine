...
namespace
{
	...
	void UT_texture_compareTGA(const char* params)
	{
		...
		LUMIX_EXPECT(file1->open("unit_tests/texture/1.tga", Lumix::FS::Mode::OPEN_AND_READ));
		LUMIX_EXPECT(file2->open("unit_tests/texture/2.tga", Lumix::FS::Mode::OPEN_AND_READ));
		LUMIX_EXPECT(Lumix::Texture::compareTGA(allocator, file1, file2, 0) == 0);

		file1->close();
		file2->close();
		LUMIX_EXPECT(file1->open("unit_tests/texture/1.tga", Lumix::FS::Mode::OPEN_AND_READ));
		LUMIX_EXPECT(file2->open("unit_tests/texture/3.tga", Lumix::FS::Mode::OPEN_AND_READ));
		LUMIX_EXPECT(Lumix::Texture::compareTGA(allocator, file1, file2, 128) == 51*51);

		file1->close();
		file2->close();
		LUMIX_EXPECT(file1->open("unit_tests/texture/1.tga", Lumix::FS::Mode::OPEN_AND_READ));
		LUMIX_EXPECT(file2->open("unit_tests/texture/4.tga", Lumix::FS::Mode::OPEN_AND_READ));
		LUMIX_EXPECT(Lumix::Texture::compareTGA(allocator, file1, file2, 1) == 512 * 512 >> 1);

		file1->close();
		file2->close();
		LUMIX_EXPECT(file1->open("unit_tests/texture/1.tga", Lumix::FS::Mode::OPEN_AND_READ));
		LUMIX_EXPECT(file2->open("unit_tests/texture/5.tga", Lumix::FS::Mode::OPEN_AND_READ));
		LUMIX_EXPECT(Lumix::Texture::compareTGA(allocator, file1, file2, 250) == 512*512);

		file1->close();
		file2->close();
		LUMIX_EXPECT(file1->open("unit_tests/texture/6.tga", Lumix::FS::Mode::OPEN_AND_READ));
		LUMIX_EXPECT(file2->open("unit_tests/texture/7.tga", Lumix::FS::Mode::OPEN_AND_READ));
		LUMIX_EXPECT(Lumix::Texture::compareTGA(allocator, file1, file2, 0) == 512*512);

		file1->close();
		file2->close();
		LUMIX_EXPECT(file1->open("unit_tests/texture/6.tga", Lumix::FS::Mode::OPEN_AND_READ));
		LUMIX_EXPECT(file2->open("unit_tests/texture/8.tga", Lumix::FS::Mode::OPEN_AND_READ));
