...
struct Runtime {
	...
	bool call(StringView function_name, Span<const Value> args, Value* result, Diagnostics& diagnostics, RuntimeOptions options = {}) {
	...
	static Value makeI32(i32 value) {
		Value v;
	...
	static Value makeU32(u32 value) {
		Value v;
	...
	static Value makeI64(i64 value) {
		Value v;
	...
	static Value makeU64(u64 value) {
		Value v;
	...
	static Value makeF32(float value) {
		Value v;
	...
	static Value makeF64(double value) {
		Value v;
	...
	static Value makeString(StringView value) {
		Value v;
	...
	static bool isBuiltinVec3(TypeRef type) {
	...
	static Value makeVec3(TypeRef type, float x, float y, float z) {
		Value v;
		v.type = type;
		v.composite[0] = x;
		v.composite[1] = y;
	...
	static Value makeQuat(TypeRef type, float x, float y, float z, float w) {
		Value v;
		v.type = type;
		v.composite[0] = x;
		v.composite[1] = y;
	...
	bool evalQualifiedEnumMember(StringView name, Value* value) {
		...
		m_diagnostics->error("Unknown enum member '", member_name, "'");
		return true;
	}

	bool step() {
		if (++m_steps <= m_options.max_steps) return true;
		m_diagnostics->error("LumScript step budget exceeded");
		return false;
	}
	...
	bool callNativeFunction(NativeFunctionDecl& fn, Span<const Value> args, Value* result) {
		if (!fn.callback) {
			m_diagnostics->error("Native function '", fn.name, "' has no callback");
			return false;
		}
		if (!fn.callback(args, result, fn.userdata)) {
			m_diagnostics->error("Native function '", fn.name, "' failed");
			return false;
		}
		return true;
	}
	...
	bool initializeGlobals() {
		...
		for (GlobalDecl& global : m_module.globals) {
			...
			if (m_diagnostics->has_error) return false;
		}
		return true;
	}
	...
	Value makeStruct(TypeRef type, Expr& e) {
		if (isBuiltinAnyVec3(type)) {
			...
			return makeVec3(type, x.f, y.f, z.f);
		}
		if (isBuiltinQuat(type)) {
			Value x = evalExpr(e.args[0]);
			Value y = evalExpr(e.args[1]);
			Value z = evalExpr(e.args[2]);
			Value w = evalExpr(e.args[3]);
			return makeQuat(type, x.f, y.f, z.f, w.f);
		}
	...
	Value evalExpr(i32 expr_idx) {
		...
		switch (e.kind) {
			...
			if (enum_idx < 0 || enum_idx >= m_module.enums.size()) {
				m_diagnostics->errorAt(e.token, "Invalid enum value");
				return {};
			}
			EnumDecl& en = m_module.enums[enum_idx];
			for (i32 i = 0; i < en.members.size(); ++i) {
	...
	bool isFloat(TypeRef::Kind kind) const {
	...
	void assign(i32 left_expr, Token::Type op, Value value) {
		...
		if (!dst) return;
		if (is_const) {
			m_diagnostics->error("Can not assign to const");
			return;
		}
		if (op == Token::EQUAL) {
			*dst = value;
