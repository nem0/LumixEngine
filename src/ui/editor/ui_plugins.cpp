...
struct UIEditorWindow : AssetEditorWindow {
		...
		, m_autocomplete_list(app.getAllocator())
	{
		...
		m_sprite_manager = LUMIX_NEW(m_app.getAllocator(), UISpriteManager)(engine);
		m_document = LUMIX_NEW(m_app.getAllocator(), ui::Document)(m_font_manager, m_app.getAllocator(), m_image_manager, m_sprite_manager);
	...
	~UIEditorWindow() {
		...
		if (m_sprite_manager) LUMIX_DELETE(m_app.getAllocator(), m_sprite_manager);
	...
	UISpriteManager* m_sprite_manager = nullptr;
...
struct UIAssetPlugin final : AssetBrowser::IPlugin, AssetCompiler::IPlugin {
	...
	void createResource(OutputMemoryStream& content) override {
		const char* template_str = R"([box width=100% height=2em padding=0.5em bg-color=#ffffff align=center font="/engine/editor/fonts/JetBrainsMono-Regular.ttf" font-size=60] { Hello World })";
