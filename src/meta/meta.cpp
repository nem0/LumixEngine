...
void serializeMain(OutputStream& out, Parser& parser) {
	L("namespace black {" OUT_ENDL);
	L("void registerLuaAPI(lua_State* L) {");
	L("	lua_newtable(L);");
	L("	lua_setglobal(L, \"black.hModules\");");

	for (Module& m : parser.modules) {
		if (m.functions.size == 0) continue;
		L("{");
		L("	lua_newtable(L);");
		L("	lua_getglobal(L, \"black.hModules\");");
		L("	lua_pushvalue(L, -2);");
	...
	// Emit enums into black.hAPI as tables so Lua can access them as black.hAPI.<EnumName>.<Member>
	for (Enum& e : parser.enums) {
		L("{");
		L("lua_getglobal(L, \"black.hAPI\");");
		L("lua_newtable(L);");
...
void serializeComponentTypes(Parser& parser) {
	...
	L("#include \"engine/black.h.h\"");
	L("namespace black::types {");
	for (Module& module : parser.modules) {
		for (Component& component : module.components) {
			L("BLACK_ENGINE_API extern const ComponentType ", component.id, ";");
		}
	}
	L("}");
...
void serializeLuaTypes(OutputStream& out_formatted) {
	...
	// Emit enum typings into black.hAPI so editors see black.hAPI.<EnumName>.<Member>
	for (Enum& e : parser.enums) {
		L("\t", e.name, " : {");
		for (Enumerator& en : e.values) {
			L("\t\t", en.name, " : number,");
...
int main() {
	...
	writeFile("src/lua/lua_capi.gen.h", lua_capi_stream);
	writeFile("data/scripts/black.h.d.lua", lua_d_stream);
