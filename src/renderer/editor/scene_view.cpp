	...
	, m_drop_handlers(app.getWorldEditor()->getAllocator())
{
	...
	m_render_interface = LUMIX_NEW(allocator, RenderInterfaceImpl)(*m_editor, *getPipeline());
	m_editor->setRenderInterface(m_render_interface);
...
{
	PROFILE_FUNCTION();
	if (&m_render_interface->m_pipeline != getPipeline())
	{
		auto& editor = *m_app.getWorldEditor();
		auto& allocator = editor.getAllocator();
		editor.setRenderInterface(nullptr);
		LUMIX_DELETE(allocator, m_render_interface);
		m_render_interface = LUMIX_NEW(allocator, RenderInterfaceImpl)(editor, *getPipeline());
