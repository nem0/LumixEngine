...
TerrainEditor::~TerrainEditor()
{
	if (m_brush_texture)
	{
		...
		BLACK_DELETE(m_app.getAllocator(), m_brush_texture);
	}
	m_app.removePlugin(*this);
	
	Engine& engine = m_app.getEngine();
	black.h::ISystem* system = engine.getSystemManager().getSystem("renderer");
	...
	if (ImGuiEx::ToolbarButton(m_app.getBigIconFont(), ICON_FA_MASK, ImGui::GetStyle().Colors[ImGuiCol_Text], "Select brush mask"))
	{
		...
		if (os::getOpenFilename(Span(filename), "All\0*.*\0", nullptr))
		{
			...
			if (data)
			{
				...
				m_brush_texture = BLACK_NEW(m_app.getAllocator(), Texture)(
