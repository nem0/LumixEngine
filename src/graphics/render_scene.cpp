...
namespace Lumix
{
	...
	public:
		...
		virtual void getRenderableInfos(Array<RenderableInfo>& infos, int64_t layer_mask) override
		{
			...
			for (int i = 0; i < m_renderables.size(); ++i)
			{
				if (m_renderables[i]->m_model.getModel() && (m_renderables[i]->m_layer_mask & layer_mask) != 0 && results[i] > -1)
				{
			...
			g_log_info.log("cull") << "render obj count: " << infos.size();
			//	<< " x = " << std::to_string(mtx.getZVector().x).c_str()
			//	<< " y = " << std::to_string(mtx.getZVector().y).c_str()
			//	<< " z = " << std::to_string(mtx.getZVector().z).c_str();
		}
