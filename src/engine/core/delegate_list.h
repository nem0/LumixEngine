...
namespace Lumix
{
	...
	class DelegateList<R(A0, A1, A2)>
	{
	public:
		DelegateList<R(A0, A1, A2)>(IAllocator& allocator)
			: m_delegates(allocator)
		{
		}
		...
		void bind()
		{
			Delegate<R(A0, A1, A2)> cb;
			cb.bind<Function>();
			m_delegates.push(cb);
		}

		template <typename C, R(C::*Function)(A0, A1, A2)>
		void bind(C* instance)
		{
			Delegate<R(A0, A1, A2)> cb;
			cb.bind<C, Function>(instance);
		...
		void invoke(A0 a0, A1 a1, A2 a2)
		{
			for (int i = 0; i < m_delegates.size(); ++i)
			{
				m_delegates[i].invoke(a0, a1, a2);
			}
		}

	private:
		Array<Delegate<R(A0, A1, A2)> > m_delegates;
	};
} // ~namespace Lumix
