...
LUMIX_ENGINE_API bool startsWith(Span<const char> str, Span<const char> prefix);
...
template <int SIZE> struct StaticString
{
	...
	void add(const Span<char>& value) { catNString(Span(data), value.begin(), value.length()); }
