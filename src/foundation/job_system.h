...
LUMIX_FOUNDATION_API bool init(u8 workers_count, IAllocator& allocator);
LUMIX_FOUNDATION_API IAllocator& getAllocator();
LUMIX_FOUNDATION_API void shutdown();
LUMIX_FOUNDATION_API u8 getWorkersCount();

LUMIX_FOUNDATION_API void enableBackupWorker(bool enable);
// yield current job and push it to worker queue
LUMIX_FOUNDATION_API void moveJobToWorker(u8 worker_index);
// yield current job, push it to global queue
LUMIX_FOUNDATION_API void yield();

LUMIX_FOUNDATION_API void enter(Mutex* mutex);
LUMIX_FOUNDATION_API void exit(Mutex* mutex);

LUMIX_FOUNDATION_API void setRed(Signal* signal);
LUMIX_FOUNDATION_API void setGreen(Signal* signal);

LUMIX_FOUNDATION_API void run(void* data, void(*task)(void*), Signal* on_finish);
LUMIX_FOUNDATION_API void runEx(void* data, void (*task)(void*), Signal* on_finish, u8 worker_index);
LUMIX_FOUNDATION_API void wait(Signal* signal);
