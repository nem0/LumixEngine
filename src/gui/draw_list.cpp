...
void DrawList::PushClipRect(Vec2 cr_min, Vec2 cr_max, bool intersect_with_current_clip_rect)
{
...
void DrawList::PushTextureID(const TextureID& texture_id)
{
...
void DrawList::ChannelsSplit(int channels_count)
{
...
void DrawList::PrimRectUV(const Vec2& a, const Vec2& c, const Vec2& uv_a, const Vec2& uv_c, u32 col)
{
...
void DrawList::PrimQuadUV(const Vec2& a, const Vec2& b, const Vec2& c, const Vec2& d, const Vec2& uv_a, const Vec2& uv_b, const Vec2& uv_c, const Vec2& uv_d, u32 col)
{
	DrawIdx idx = (DrawIdx)_VtxCurrentIdx;
	_IdxWritePtr[0] = idx; _IdxWritePtr[1] = (DrawIdx)(idx + 1); _IdxWritePtr[2] = (DrawIdx)(idx + 2);
	_IdxWritePtr[3] = idx; _IdxWritePtr[4] = (DrawIdx)(idx + 2); _IdxWritePtr[5] = (DrawIdx)(idx + 3);
...
void DrawList::AddPolyline(const Vec2* points, const int points_count, u32 col, bool closed, float thickness, bool anti_aliased)
{
	if (points_count < 2)
		return;
...
void DrawList::AddConvexPolyFilled(const Vec2* points, const int points_count, u32 col, bool anti_aliased)
{
	const Vec2 uv = FontTexUvWhitePixel;
...
void DrawList::PathArcToFast(const Vec2& centre, float radius, int amin, int amax)
{
...
void DrawList::PathArcTo(const Vec2& centre, float radius, float amin, float amax, int num_segments)
{
	if (radius == 0.0f)
		_Path.push(centre);
...
void DrawList::PathBezierCurveTo(const Vec2& p2, const Vec2& p3, const Vec2& p4, int num_segments)
{
	Vec2 p1 = _Path.back();
	if (num_segments == 0)
	{
...
void DrawList::PathRect(const Vec2& a, const Vec2& b, float rounding, int rounding_corners)
{
	float r = rounding;
...
void DrawList::AddLine(const Vec2& a, const Vec2& b, u32 col, float thickness)
{
...
void DrawList::AddRect(const Vec2& a, const Vec2& b, u32 col, float rounding, int rounding_corners_flags, float thickness)
{
	if ((col & IM_COL32_A_MASK) == 0)
		return;
...
void DrawList::AddRectFilled(const Vec2& a, const Vec2& b, u32 col, float rounding, int rounding_corners_flags)
{
	if ((col & IM_COL32_A_MASK) == 0)
		return;
	if (rounding > 0.0f)
...
void DrawList::AddRectFilledMultiColor(const Vec2& a, const Vec2& c, u32 col_upr_left, u32 col_upr_right, u32 col_bot_right, u32 col_bot_left)
{
...
void DrawList::AddQuad(const Vec2& a, const Vec2& b, const Vec2& c, const Vec2& d, u32 col, float thickness)
{
	if ((col & IM_COL32_A_MASK) == 0)
		return;
...
void DrawList::AddQuadFilled(const Vec2& a, const Vec2& b, const Vec2& c, const Vec2& d, u32 col)
{
	if ((col & IM_COL32_A_MASK) == 0)
		return;
...
void DrawList::AddTriangle(const Vec2& a, const Vec2& b, const Vec2& c, u32 col, float thickness)
{
	if ((col & IM_COL32_A_MASK) == 0)
		return;
...
void DrawList::AddCircle(const Vec2& centre, float radius, u32 col, int num_segments, float thickness)
{
	if ((col & IM_COL32_A_MASK) == 0)
		return;
...
void DrawList::AddCircleFilled(const Vec2& centre, float radius, u32 col, int num_segments)
{
	if ((col & IM_COL32_A_MASK) == 0)
		return;
...
void DrawList::AddBezierCurve(const Vec2& pos0, const Vec2& cp0, const Vec2& cp1, const Vec2& pos1, u32 col, float thickness, int num_segments)
{
	if ((col & IM_COL32_A_MASK) == 0)
		return;
...
void DrawList::AddText(const Font* font, float font_size, const Vec2& pos, u32 col, const char* text_begin, const char* text_end, float wrap_width, const Vec4* cpu_fine_clip_rect)
{
	if ((col & IM_COL32_A_MASK) == 0)
		return;
	...
	ASSERT(font->ContainerAtlas->TexID == _TextureIdStack.back());  // Use high-level ImGui::PushFont() or low-level DrawList::PushTextureId() to change font.
...
void DrawList::AddImage(TextureID user_texture_id, const Vec2& a, const Vec2& b, const Vec2& uv0, const Vec2& uv1, u32 col)
{
	if ((col & IM_COL32_A_MASK) == 0)
		return;
...
void Font::RenderChar(DrawList* draw_list, float size, Vec2 pos, u32 col, unsigned short c) const
{
...
void Font::RenderText(DrawList* draw_list, float size, Vec2 pos, u32 col, const Vec4& clip_rect, const char* text_begin, const char* text_end, float wrap_width, bool cpu_fine_clip) const
{
	if (!text_end)
	...
	DrawList::DrawVert* vtx_write = draw_list->_VtxWritePtr;
	DrawList::DrawIdx* idx_write = draw_list->_IdxWritePtr;
		...
		if (const Glyph* glyph = FindGlyph((unsigned short)c))
		{
			...
			if (c != ' ' && c != '\t')
			{
				...
				if (x1 <= clip_rect.z && x2 >= clip_rect.x)
				{
					...
					// Inlined here:
					{
						idx_write[0] = (DrawList::DrawIdx)(vtx_current_idx); idx_write[1] = (DrawList::DrawIdx)(vtx_current_idx + 1); idx_write[2] = (DrawList::DrawIdx)(vtx_current_idx + 2);
						idx_write[3] = (DrawList::DrawIdx)(vtx_current_idx); idx_write[4] = (DrawList::DrawIdx)(vtx_current_idx + 2); idx_write[5] = (DrawList::DrawIdx)(vtx_current_idx + 3);
