...
// ImDrawList and dependencies from imgui + refactored
namespace Lumix
{
...
struct Font
{
	...
	void              RenderChar(DrawList* draw_list, float size, Vec2 pos, u32 col, unsigned short c) const;
	void              RenderText(DrawList* draw_list, float size, Vec2 pos, u32 col, const Vec4& clip_rect, const char* text_begin, const char* text_end, float wrap_width = 0.0f, bool cpu_fine_clip = false) const;
...
struct DrawList
{
	...
	struct DrawCmd
	{
		unsigned int    ElemCount;              // Number of indices (multiple of 3) to be rendered as triangles. Vertices are stored in the callee DrawList's vtx_buffer[] array, indices in idx_buffer[].
	...
	DrawList(IAllocator& _allocator)
