...
namespace Lux 
{
	namespace MT
	{
		template <class T, int32_t size> class LockFreeFIFO 
		{
		public:
		...
		template <class T, int32_t size>		LockFreeFIFO<T, size>::LockFreeFIFO()
			: m_rd(0)
			, m_wr(0)
		{
			ASSERT(size > 0);
		...
		template <class T, int32_t size> bool LockFreeFIFO<T, size>::isEmpty() const
		{
			return m_wr == m_rd;
		}

		template <class T, int32_t size> int32_t LockFreeFIFO<T, size>::push(T* data)
		{
			ASSERT(NULL != data);
		...
		template <class T, int32_t size> T* LockFreeFIFO<T, size>::pop()
		{
			while (m_rd != m_wr)
			{
