...
TEST(BytecodeTypeMetadataExposesSizeAndAlignment) {
	...
	EXPECT_EQ(16u, ex_type_get_size(data));
	EXPECT_EQ(8u, ex_type_get_alignment(data));
	EXPECT_EQ(0u, ex_type_get_alignment(nullptr));
	CAPI_END(module);
	return true;
}
...
TEST(AttributeMetadataIsAvailableThroughCAPI) {
	...
	for (u32 i = 0; i < ex_debug_frame_local_count(runtime, 0); ++i) {
		if (equalStrings(ex_debug_local_name(runtime, 0, i), toLs("settings"))) {
			settings_type = ex_debug_local_type(runtime, 0, i);
			break;
		}
	}
	...
	i32 min = 0;
	i32 max = 0;
	memcpy(&min, (const u8*)type_attribute.value + ex_type_struct_field_offset(type_attribute.type, 0), sizeof(min));
	memcpy(&max, (const u8*)type_attribute.value + ex_type_struct_field_offset(type_attribute.type, 1), sizeof(max));
	EXPECT_EQ(1, min);
	EXPECT_EQ(9, max);
	...
	EXPECT_EQ(0u, ex_type_attribute_count(nullptr));
	ex_attribute invalid = ex_type_attribute_value(nullptr, 0);
	EXPECT_TRUE(invalid.type == nullptr);
	EXPECT_TRUE(invalid.value == nullptr);
	invalid = ex_type_attribute_value(settings_type, 1);
	EXPECT_TRUE(invalid.type == nullptr);
	EXPECT_TRUE(invalid.value == nullptr);
	invalid = ex_type_struct_field_attribute_value(settings_type, 0, 1);
	EXPECT_TRUE(invalid.type == nullptr);
	EXPECT_TRUE(invalid.value == nullptr);
	EXPECT_EQ(0u, ex_type_struct_field_attribute_count(settings_type, 1));
	EXPECT_EQ(0u, ex_type_struct_field_attribute_count(nullptr, 0));
	invalid = ex_type_struct_field_attribute_value(nullptr, 0, 0);
	EXPECT_TRUE(invalid.type == nullptr);
	EXPECT_TRUE(invalid.value == nullptr);
	const ex_type* value_type = ex_type_struct_field_type(settings_type, 0);
	EXPECT_EQ(0u, ex_type_attribute_count(value_type));
	EXPECT_EQ(0u, ex_type_struct_field_attribute_count(value_type, 0));
	invalid = ex_type_struct_field_attribute_value(value_type, 0, 0);
	EXPECT_TRUE(invalid.type == nullptr);
	EXPECT_TRUE(invalid.value == nullptr);
