...
TEST(NativeFunctionsAreEnumeratedByUnit) {
	...
	EXPECT_TRUE(unit);
	EXPECT_TRUE(equalStrings(ex_unit_get_path(unit), __func__));
	EXPECT_EQ(2, ex_unit_get_native_function_count(unit));
	EXPECT_TRUE(equalStrings(ex_unit_get_native_function_name(unit, 0), "first"));
	EXPECT_TRUE(equalStrings(ex_unit_get_native_function_name(unit, 1), "second"));
	CAPI_END(module);
	return true;
}
