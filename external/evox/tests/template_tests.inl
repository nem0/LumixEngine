...
TEST(FailedImportedFactoryInstanceRemainsInvalid) {
	...
	EXPECT_TRUE(ex_module_parse(module, toLs(first_source), toLs("lib")) == EX_RESULT_OK);
	EXPECT_TRUE(ex_module_typecheck(module) == EX_RESULT_FAILURE);
	EXPECT_TRUE(ex_module_parse(module, toLs(second_source), toLs("main")) == EX_RESULT_OK);
	EXPECT_TRUE(ex_module_typecheck(module) == EX_RESULT_FAILURE);
