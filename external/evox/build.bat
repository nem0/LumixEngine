...
REM build.bat - Build evoxc.exe standalone Evox runner using cl.exe or clang-cl
REM
REM Usage:
REM   build.bat
REM   build.bat tests
...
if /I "%TARGET%"=="tests" (
    set CFLAGS=%CFLAGS% /Od /MDd /DEX_TESTS
    set CXXFLAGS=%CXXFLAGS% /Od /MDd /DEX_TESTS
) else (
    if /I "%CONFIG%"=="release" (
...
if /I "%TARGET%"=="tests" (
    ...
    set LDFLAGS=/nologo /DEBUG /INCREMENTAL:NO /OUT:"%OUT_DIR%\evoxc.exe"
)
...
if /I "%TARGET%"=="tests" (
	%COMPILER% %CXXFLAGS% /c "%SCRIPT_DIR%tests/main.cpp" "%SCRIPT_DIR%parser.cpp" "%SCRIPT_DIR%formatter.cpp" "%SCRIPT_DIR%compiler.cpp" "%SCRIPT_DIR%ir.cpp" "%SCRIPT_DIR%capi.cpp" "%SCRIPT_DIR%platform.cpp"
	if errorlevel 1 exit /b !errorlevel!
	%COMPILER% %CFLAGS% /c "%SCRIPT_DIR%runtime.c" "%SCRIPT_DIR%debugger.c"
	if errorlevel 1 exit /b !errorlevel!
	%COMPILER% "main.obj" "parser.obj" "formatter.obj" "compiler.obj" "ir.obj" "runtime.obj" "debugger.obj" "capi.obj" "platform.obj" /link %LDFLAGS%
) else (
	if /I "%USE_CLANG%"=="1" (
		%COMPILER% %CFLAGS% /c "%SCRIPT_DIR%evoxc.c"
		if errorlevel 1 exit /b !errorlevel!
        %COMPILER% %CXXFLAGS% /c "%SCRIPT_DIR%parser.cpp" "%SCRIPT_DIR%formatter.cpp" "%SCRIPT_DIR%compiler.cpp" "%SCRIPT_DIR%ir.cpp" "%SCRIPT_DIR%capi.cpp" "%SCRIPT_DIR%platform.cpp"
		if errorlevel 1 exit /b !errorlevel!
		%COMPILER% %CFLAGS% /c "%SCRIPT_DIR%runtime.c" "%SCRIPT_DIR%debugger.c"
		if errorlevel 1 exit /b !errorlevel!
        %COMPILER% "evoxc.obj" "parser.obj" "formatter.obj" "compiler.obj" "ir.obj" "runtime.obj" "debugger.obj" "capi.obj" "platform.obj" /link %LDFLAGS%
	) else (
		%COMPILER% %CFLAGS% /c "%SCRIPT_DIR%evoxc.c"
		if errorlevel 1 exit /b !errorlevel!
        %COMPILER% %CXXFLAGS% /c "%SCRIPT_DIR%parser.cpp" "%SCRIPT_DIR%formatter.cpp" "%SCRIPT_DIR%compiler.cpp" "%SCRIPT_DIR%ir.cpp" "%SCRIPT_DIR%capi.cpp" "%SCRIPT_DIR%platform.cpp"
