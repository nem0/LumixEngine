...
if /I "%TARGET%"=="tests" (
	%COMPILER% %CXXFLAGS% /c "%SCRIPT_DIR%tests/main.cpp" "%SCRIPT_DIR%parser.cpp" "%SCRIPT_DIR%compiler.cpp" "%SCRIPT_DIR%ir.cpp" "%SCRIPT_DIR%capi.cpp" "%SCRIPT_DIR%platform.cpp"
	if errorlevel 1 exit /b !errorlevel!
	%COMPILER% %CFLAGS% /c "%SCRIPT_DIR%runtime.c" "%SCRIPT_DIR%debugger.c"
	if errorlevel 1 exit /b !errorlevel!
	%COMPILER% "main.obj" "parser.obj" "compiler.obj" "ir.obj" "runtime.obj" "debugger.obj" "capi.obj" "platform.obj" /link %LDFLAGS%
) else (
	if /I "%USE_CLANG%"=="1" (
		%COMPILER% %CFLAGS% /c "%SCRIPT_DIR%evoxc.c"
		if errorlevel 1 exit /b !errorlevel!
        %COMPILER% %CXXFLAGS% /c "%SCRIPT_DIR%parser.cpp" "%SCRIPT_DIR%compiler.cpp" "%SCRIPT_DIR%ir.cpp" "%SCRIPT_DIR%capi.cpp" "%SCRIPT_DIR%platform.cpp"
		if errorlevel 1 exit /b !errorlevel!
		%COMPILER% %CFLAGS% /c "%SCRIPT_DIR%runtime.c" "%SCRIPT_DIR%debugger.c"
		if errorlevel 1 exit /b !errorlevel!
        %COMPILER% "evoxc.obj" "parser.obj" "compiler.obj" "ir.obj" "runtime.obj" "debugger.obj" "capi.obj" "platform.obj" /link %LDFLAGS%
	) else (
		%COMPILER% %CFLAGS% /c "%SCRIPT_DIR%evoxc.c"
		if errorlevel 1 exit /b !errorlevel!
        %COMPILER% %CXXFLAGS% /c "%SCRIPT_DIR%parser.cpp" "%SCRIPT_DIR%compiler.cpp" "%SCRIPT_DIR%ir.cpp" "%SCRIPT_DIR%capi.cpp" "%SCRIPT_DIR%platform.cpp"
