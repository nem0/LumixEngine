...
static int debug_string_equals(ex_string_view a, ex_string_view b) {
...
// how it was reached: a suspend (EX_OP_BREAK, or the error-suspend path in
...
static int debug_find_breakpoint_target(const ex_bytecode* bytecode, ex_string_view source_name, u32 line, u32* out_function_index, u32* out_code_offset, u32* out_line) {
	int found = 0;
	u32 best_line = 0u;
...
ex_result ex_debug_set_breakpoint(ex_bytecode* bytecode, ex_string_view source_name, u32 line, u32* resolved_line) {
	...
	u32 code_offset = 0u;
	u32 found_line = 0u;
	if (!debug_find_breakpoint_target(bytecode, source_name, line, &function_index, &code_offset, &found_line)) return EX_RESULT_FAILURE;
	...
	if (bytecode->breakpoint_count == bytecode->breakpoint_capacity) {
		...
		ex_bytecode_breakpoint* grown = (ex_bytecode_breakpoint*)realloc(bytecode->breakpoints, sizeof(ex_bytecode_breakpoint) * new_capacity);
		if (!grown) return EX_RESULT_FAILURE;
		bytecode->breakpoints = grown;
		bytecode->breakpoint_capacity = new_capacity;
	}

	ex_bytecode_breakpoint* bp = &bytecode->breakpoints[bytecode->breakpoint_count++];
	bp->code = code;
...
void ex_debug_remove_all_breakpoints(ex_bytecode* bytecode) {
	if (!bytecode) return;
	for (u32 i = 0; i < bytecode->breakpoint_count; ++i) {
		const ex_bytecode_breakpoint* bp = &bytecode->breakpoints[i];
		*bp->code = bp->original_byte;
	}
...
static const ex_bytecode_local_debug_entry* debug_local_at(ex_runtime* runtime, u32 frame_index, u32 local_index, u32* out_visible_count) {
	...
	for (u32 i = 0; i < frame->function->local_count; ++i) {
		const ex_bytecode_local_debug_entry* entry = &frame->function->locals[i];
...
ex_type_kind ex_type_get_kind(const ex_type* type) {
	return type ? type->kind : EX_TYPE_INVALID;
}
...
u32 ex_type_struct_field_count(const ex_type* type) {
	return (type && type->kind == EX_TYPE_STRUCT) ? type->field_count : 0u;
}

ex_string_view ex_type_struct_field_name(const ex_type* type, u32 field_index) {
	const ex_string_view empty = {NULL, 0};
	if (!type || type->kind != EX_TYPE_STRUCT || !type->bytecode) return empty;
...
const ex_type* ex_type_struct_field_type(const ex_type* type, u32 field_index) {
	if (!type || type->kind != EX_TYPE_STRUCT || !type->bytecode) return NULL;
	...
	return (const ex_type*)&type->bytecode->type_info[ti];
}

u32 ex_type_struct_field_offset(const ex_type* type, u32 field_index) {
	if (!type || type->kind != EX_TYPE_STRUCT || !type->bytecode) return 0u;
...
u32 ex_type_union_member_count(const ex_type* type) {
	return (type && type->kind == EX_TYPE_TAGGED_UNION) ? type->member_count : 0u;
}

const ex_type* ex_type_union_member_type(const ex_type* type, u32 member_index) {
	if (!type || type->kind != EX_TYPE_TAGGED_UNION || !type->bytecode) return NULL;
...
u32 ex_type_enum_value_count(const ex_type* type) {
	return (type && type->kind == EX_TYPE_ENUM) ? type->value_count : 0u;
}

ex_string_view ex_type_enum_value_name(const ex_type* type, u32 value_index) {
	const ex_string_view empty = {NULL, 0};
	if (!type || type->kind != EX_TYPE_ENUM || !type->bytecode) return empty;
...
const ex_type* ex_type_array_element_type(const ex_type* type) {
	...
	if (type->kind != EX_TYPE_ARRAY && type->kind != EX_TYPE_SLICE) return NULL;
...
const ex_type* ex_type_nullable_inner_type(const ex_type* type) {
	if (!type || !type->bytecode) return NULL;
	if (type->kind != EX_TYPE_NULLABLE) return NULL;
	if (type->element_type_index >= type->bytecode->type_info_count) return NULL;
	return (const ex_type*)&type->bytecode->type_info[type->element_type_index];
}
