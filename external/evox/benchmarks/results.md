...
Single-run wall-clock times of the Evox bytecode interpreter against
...
- OS: Windows 11 Home
- evoxc: release build (`build.bat release`, MSVC 19.51, `/O2`)
...
| Benchmark | Workload | Result | evoxc | lua 5.4 | node --jitless | node | C /O2 |
|---|---|---|---:|---:|---:|---:|---:|
...
Relative to evoxc (higher = faster than evoxc):
...
| Benchmark | Evox | Lua 5.4 | Node.js V8 |
|---|---:|---:|---:|
| recursion | 16 | 44 | 73 |
| mandel | 68 | 124 | 165 |
| fannkuch | 81 | 147 | 207 |
...
Evox counts are executable instructions from `evoxc --dump-bytecode`; Lua
...
Each chart uses the same engine order: evoxc, Lua 5.4, Node.js jitless,
Node.js JIT, and C `/O2`.

```mermaid
xychart-beta
    title "recursion: fib(30)"
    x-axis [evoxc, lua, jitless, node, C]
...
```
run.bat            evoxc (builds release evoxc.exe first)
