...
TEST(Extern) {
	...
	CAPI_RUNTIME(module, runtime);
	EXPECT_TRUE(setNativeFunctionCallback(runtime, module, toLs("nativefn"), nativefn) == LS_RESULT_OK);
	EXPECT_TRUE(setNativeFunctionCallback(runtime, module, toLs("nativefn2"), nativefn2) == LS_RESULT_OK);
	EXPECT_TRUE(ls_call(runtime, toLs("main")));
	EXPECT_EQ(42, ls_to_i32(runtime, -1));
	CAPI_END(module);
...
TEST(NativeStringResult) {
	...
	CAPI_RUNTIME(module, runtime);
	EXPECT_TRUE(setNativeFunctionCallback(runtime, module, toLs("getText"), get_text) == LS_RESULT_OK);
	EXPECT_TRUE(ls_call(runtime, toLs("main")));
	EXPECT_EQ(42, ls_to_i32(runtime, -1));
	CAPI_END(module);
	return true;
...
TEST(BytecodeNestedDefer) {
	...
	EXPECT_TRUE(ls_call(runtime, toLs("main")));
	EXPECT_EQ(0, ls_to_i32(runtime, -1));
	EXPECT_EQ(1, ls_to_i32(runtime, 0)); // a
	EXPECT_EQ(2, ls_to_i32(runtime, 1)); // b
	CAPI_END(module);
	return true;
}
...
TEST(UntypedLiteralsRuntime) {
	...
	CAPI_BEGIN(module, diagnostics);
	bool compiled = ls_module_compile(module, toLs(source), makeStringView(__func__), nullptr, nullptr);
...
TEST(FirstClassFunctionsRuntime) {
	...
	CAPI_BEGIN(module, diagnostics);
	const bool ok = ls_module_compile(module, toLs(source), makeStringView(__func__), nullptr, nullptr);
	EXPECT_TRUE(ok);
