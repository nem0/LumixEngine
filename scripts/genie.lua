...
function buildPluginDefines()
	if #plugins > 0 and dynamic_plugins then
		...
		defines { "BLACKENGINE_PLUGINS=" .. def }
	end
end
...
if plugin "physics" then
	...
	defines { "BUILDING_PHYSICS", "BLACK_STATIC_PHYSX" }
...
io.write "#ifdef BLACK_PLUGIN_DECLS\n"
	for _, plugin in ipairs(plugin_creators) do
		io.write([[extern "C" ISystem* createPlugin_]] .. plugin .. "(Engine&);\n")
	end
io.write "#elif defined BLACK_EDITOR_PLUGINS_DECLS\n"
	if not dynamic_plugins then
		for _, plugin in ipairs(plugin_creators) do
			io.write([[extern "C" black.h::StudioApp::IPlugin* setStudioApp_]] .. plugin .. "(StudioApp&);\n")
		end
	end
io.write "#elif defined BLACK_EDITOR_PLUGINS\n"
	if not dynamic_plugins then
		for _, plugin in ipairs(plugin_creators) do
			io.write "{\n"
...
io.write "#elif defined BLACK_PLUGINS_STRINGS\n"
	if dynamic_plugins then
		for _, plugin in ipairs(plugin_creators) do
			io.write("\"" .. plugin .. "\", ")
		end
