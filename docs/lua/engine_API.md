...
The API can be found in [black.h.d.lua](../../data/scripts/black.h.d.lua). The file is generated by the project's Meta tool at build-time (see [src/meta/meta.cpp](src/meta/meta.cpp#L2348)). For the most accurate, up-to-date API use that generated file; the list below is a concise reference of commonly used `black.hAPI` bindings present in the source.
...
```lua
black.hAPI.logError("Hello world")
```